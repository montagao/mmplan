...
func (s *PlanStore) GetByID(id int64) (*models.Plan, error) {
	...
	err = rows.Scan(&nullID, &isComplete, &planName, &list1, &list2, &name1, &name2, &timestamp)
	if err != nil {
		return nil, err
	}
	e := &models.Plan{
...
func (s *PlanStore) updateList1(plan *models.Plan) error {
	...
	_, err = stmt.Exec(plan.ID, plan.List1, plan.Name1, plan.Timestamp, plan.IsComplete, plan.PlanName)
	return err
}
