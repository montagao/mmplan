...
func (s *PlanStore) updateList1(plan *models.Plan) error {
	stmt, err := s.db.Prepare("INSERT INTO plans(list1, name1, timestamp, iscomplete) VALUES( $1, $2, $3, $4 ) ON CONFLICT id DO UPDATE;")
...
func (s *PlanStore) updateList2(plan *models.Plan) error {
	stmt, err := s.db.Prepare("INSERT INTO plans(list2, name2, timestamp, iscomplete) VALUES( $1, $2, $3, $4 ) ON CONFLICT id DO UPDATE;")
	// Prepared statements take up server resources and should be closed after use.
	defer stmt.Close()
